(function (global, factory) {
	...
	(global.d3_timeseries = factory(global.d3));
...
var d3_timeseries = function () {
  ...
  function createLines(serie) {
    ...
    serie.interpolationFunction = d3[curveName] || function () {
      console.error(`no suche interpolation function d3.${curveName}`);
      return d3.curveLinear;
    }();

    var line$$1 = d3.line().x(functorkeyscale(aes.x, xscale)).y(functorkeyscale(aes.y, yscale)).curve(serie.interpolationFunction).defined(keyNotNull(aes.y));
  ...
  function drawSerie(serie) {
    if (!serie.linepath) {
      ...
      if (serie.diffAreas) {
        serie.diffpaths = serie.diffAreas.map(function (area$$1, i) {
          ...
          }).datum(serie.data).attr('class', 'd3_timeseries diff-area').attr('d', area$$1).attr('stroke', 'none').attr('fill', c).attr('opacity', serie.options.diff_opacity || 0.5);
        });
      }
    } else {
  ...
  function drawMiniDrawer() {
    ...
    var serie = series[0];
    var line$$1 = d3.line().x(functorkeyscale(serie.aes.x, fullxscale)).y(functorkeyscale(serie.aes.y, smallyscale)).curve(serie.interpolationFunction).defined(keyNotNull(serie.aes.y));
    var linepath = drawerContainer.insert("path", ":first-child").datum(serie.data).attr('class', 'd3_timeseries.line').attr('transform', 'translate(0,' + drawerTopMargin + ')').attr('d', line$$1).attr('stroke', serie.options.color).attr('stroke-width', serie.options.width || 1.5).attr('fill', 'none');
  ...
  var chart = function (elem) {
    ...
    series = series.map(function (s) {
      var extent$$1 = d3.extent(s.data.map(functorkey(s.aes.y)));
      s.min = extent$$1[0];
      s.max = extent$$1[1];
      extent$$1 = d3.extent(s.data.map(functorkey(s.aes.x)));
      s.dateMin = extent$$1[0];
      s.dateMax = extent$$1[1];
      return s;
    });
    ...
    brush.extent([[fullxscale.range()[0], 0], [fullxscale.range()[1], drawerHeight - drawerTopMargin]]).on('brush', () => {
    ...
    drawerContainer.append("g").attr("class", "d3_timeseries brush").call(brush).attr('y', drawerTopMargin).attr("height", drawerHeight - drawerTopMargin);
